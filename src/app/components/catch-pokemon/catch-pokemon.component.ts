...
export class CatchPokemonComponent implements OnInit {
  ...
  constructor(private speciesService: SpeciesService, private http: HttpClient, private urlService: UrlService, private authenticationService: AuthenticationService, private pokemonService: PokemonService) { }

  ngOnInit() {
    this.currentAccount = this.authenticationService.getAccount();
    this.speciesService.findWhere("foundAt", this.currentMap.name).get().then((data: ListResponse<Species>) => {
  ...
  catchRandom() {
    let rand = Math.floor(Math.random() * this.species.length);
    ...
    if (Math.random() > 0.5) {
      ...
      // POST /pokemon 
      this.pokemonService.create(pokemon).then(data => {
  ...
  setBallToUsed() {
    return new Promise((res, rej) => {
      // PUT /item
      this.http.put(this.urlService.getEndpoint() + "/item", {
